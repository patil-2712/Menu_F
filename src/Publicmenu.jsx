...
//     if (!customerPhone.trim() || !validatePhone(customerPhone.trim())) {
//       setPhoneError(true);
//       showPopup('⚠️ Please enter a valid 10-digit mobile number', 'error');
//       return;
//     }
...
//       {/* Order Summary with Customer Name, Phone and Table Number Fields */}
...
//                   {/* Customer Name, Phone and Table Number Input Fields */}
...
//                         />
//                       </div>
//                       {phoneError && <span className="error-text-order">Please enter valid 10-digit mobile number</span>}
//                     </div>
  ...
  const getUserLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        // CRITICAL FIX 1: Block if restaurant location is missing
        if (!currentRestaurant || !currentRestaurant.latitude || !currentRestaurant.longitude) {
          console.log('⚠️ Restaurant location missing, blocking order');
          setLocationStatus('outside_range');
          setLocationError('Restaurant location not configured. Please contact staff.');
          return;
        }
        ...
        // CRITICAL FIX 2: Block if coordinates are invalid
        if (isNaN(restLat) || isNaN(restLon)) {
          console.log('⚠️ Restaurant coordinates invalid, blocking order');
          setLocationStatus('outside_range');
          setLocationError('Invalid restaurant coordinates. Please contact staff.');
          return;
        }
        
        console.log('📍 Restaurant location:', restLat, restLon);
        
        const distance = calculateDistance(latitude, longitude, restLat, restLon);
        setDistanceToRestaurant(distance);
        console.log('📏 Distance to restaurant:', Math.round(distance), 'meters');
        
        // CRITICAL FIX 3: Use local 'distance' variable, not state
        const ALLOWED_DISTANCE = 40; // Change to 300 if you want 300 meters
        ...
        if (distance <= ALLOWED_DISTANCE) {
          ...
          setLocationStatus('outside_range');
          console.log(`❌ Outside range! ${Math.round(distance)}m > ${ALLOWED_DISTANCE}m`);
        }
      },
      (error) => {
        console.error('Location error:', error);
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        total: Number(getGrandTotal()),
        status: 'pending',
        // Add user location and IP for security tracking
        userLatitude: userLocation?.latitude || null,
        userLongitude: userLocation?.longitude || null,
        distanceToRestaurant: distanceToRestaurant || null,
        userIP: userIP  // IP address saved with order
      };
